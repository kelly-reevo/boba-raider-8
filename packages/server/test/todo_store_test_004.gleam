...
pub fn list_all_returns_items_in_creation_order_test() {
  let assert Ok(store) = todo_store.start()
  let assert Ok(_item1) = todo_store.create_todo(store, "First", "")
  process.sleep(10)
  let assert Ok(_item2) = todo_store.create_todo(store, "Second", "")
  process.sleep(10)
  let assert Ok(_item3) = todo_store.create_todo(store, "Third", "")
  let all = todo_store.get_all_todos(store)
  should.equal(list.length(all), 3)
