...
/// Fetch all todos using JavaScript fetch
...
fn parse_todos(json_string: String) -> Result(List(Todo), Nil) {
  // For simplicity, we'll use a basic approach - parse manually from the JSON
  // Since the decoder API is complex, we use the FFI to parse and return structured data
