...
pub fn list_all_returns_all_stored_items_test() {
  ...
  let assert Ok(item1) = todo_store.create_todo(store, "First task", "")
  let assert Ok(item2) = todo_store.create_todo(store, "Second task", "")
  let assert Ok(item3) = todo_store.create_todo(store, "Third task", "")
...
pub fn list_all_returns_items_in_creation_order_test() {
  let assert Ok(store) = todo_store.start()
  let assert Ok(item1) = todo_store.create_todo(store, "First", "")
  process.sleep(10)
  let assert Ok(item2) = todo_store.create_todo(store, "Second", "")
  process.sleep(10)
  let assert Ok(item3) = todo_store.create_todo(store, "Third", "")
  let all = todo_store.get_all_todos(store)
  should.equal(list.length(all), 3)
  let timestamps = list.map(all, fn(t) { t.created_at })
  let sorted_timestamps = list.sort(timestamps, int.compare)
