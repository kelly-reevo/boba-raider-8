// Integration test for PATCH /api/todos/:id - validation error handling
...
pub fn patch_todo_empty_title_returns_422_test() {
  ...
  let title_errors = errors |> list.filter(fn(e) { e.0 == "title" })
  should.be_true(!list.is_empty(title_errors))
}
